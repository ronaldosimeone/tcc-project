...
function riskLevel(prob) {
  if (prob < 0.3) return { label: "LOW", color: GREEN };
  if (prob < 0.7) return { label: "MÉDIO", color: YELLOW };
...
async function runTest({ label, payload }) {
  ...
  try {
    ...
    console.error(
      `${RED}[ERRO]${RESET} Não foi possível conectar em ${API_URL}`,
    );
    console.error(
      `${DIM}       Certifique-se de que a stack está no ar: docker compose up${RESET}`,
    );
    process.exit(1);
  }
...
(async () => {
  ...
  console.log(
    `${CYAN}${BOLD}  MetroPT-3 · Teste de Inferência Real via FastAPI${RESET}`,
  );
  console.log(
    `${CYAN}${BOLD}══════════════════════════════════════════════════════${RESET}`,
  );
